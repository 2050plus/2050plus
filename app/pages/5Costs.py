...
with col1:
    selected_cost_segment = st.selectbox('Choose your segment :', ["Total"] + list(df_cost_segments.cost_segment.unique()))
...
st.markdown("The marginal price represents the cost of an additional unit of an energy carrier at one country at a given time. Please note that the shown value does not take into account taxes nor trading dynamic.")
...
with col1:
    ...
    df1 = df1.rename(columns = lambda x : x +' Annual average ' if not(x.endswith('_std')) else x.replace('_std', ' Standard Deviation')).T
...
with col2:
    ...
    df2 = df2.rename(columns = lambda x : x +' Annual average ' if not(x.endswith('_std')) else x.replace('_std', ' Standard Deviation')).T
